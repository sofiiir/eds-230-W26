...
Ptime <- ode(y = initial_rabbits, times = years, func = dexppop, parms = c(0.1))
...
growth_result$pop_analytic <- growth_result$time |> 
  map_dbl(~exppop(P0 =  initial_pop, r = r, T = .x))
...
growth_result$Pdiscrete <- growth_result$time |> 
  map_dbl(~discrete_logistic_pop(P0= initial_pop, r = r, T = .x))

# add a column for ODE result - two steps
tmp <- ode(y = initial_pop, times = growth_result$time, dexppop, parms = c(r))
growth_result$Pode <- tmp[,2]

# gather both model results using a key called "Ptype", value "P"
grnice <- growth_result |> 
  gather(key = "method", value = "pop", ~time) # gather is the same as pivot longer
