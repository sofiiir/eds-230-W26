...
harvestr = seq(from = 0 , to = 0.4 , by = 0.025)
allres <- harvestr |> 
  map_dfc(~getcarbon(Cinitial = 20, tm = tm, K = 100 , r = 0.2, hfunc = dharvest, harv = .x))
...
allres <- allres |> 
  pivot_longer(names_to = "harvestr", 
               values_to = "carbon", 
               -time)
# add time
...
# graph
ggplot(allres, aes(time, carbon, col = harvestr)) +
  geom_line() +
  theme_minimal()
