...
Models will often have a data structure (or file) that parameter values to explore \* could build a function to read this in and run the model for each parameter set
...
[*IF*]{style="color: blue;"} **P** and **T** were in the same units (e.g percent deviation from a mean value), then you could use the coefficients to tell you if parrot suitability would be more sensitive to a 10% change in precipitation versus a 10% change in temperature
