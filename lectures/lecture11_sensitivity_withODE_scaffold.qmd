...
compute_pop_metrics <- function(result, thresh) {
  ...
  idx <- which(result$P > thresh)[1] # the one is taking just the first one
...
p_wrapper <- function(r, K, Pinitial, simtimes, odefunc, metricfunc, metricpars) {
  ...
  # get metrics
  metrics <- metricfunc(result, metricpars$thresh)
...
p_wrapper(r = 0.01, K= 150, Pinitial = 3, simtimes = seq(from = 1, to = 10), odefunc = dpopgrowth, metricfunc = compute_pop_metrics, metricpars = metricpars)
...
str(sens_P)
sens_P_threshyear <- sensitivity::tell(sens_P, allres$threshyear) # calls sobolsalt again
...
ggplot(allresp, aes(K,threshyear, col=r )) +
  geom_point() +
  labs(y="Year that threshold is reached", 
       x="K")
```
