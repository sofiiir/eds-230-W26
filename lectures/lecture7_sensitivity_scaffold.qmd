...
pnorm(0.55, mean = 5, sd = 2)
# there is a one percent chance of erosion 

# Models should be run on large samples to analyze true effects and any outliers.
```
...
# values come from the regression 
parm[, "Tmincoeff2"] <- qnorm(parm_quant[, "Tmincoeff2"], mean = -0.00846, sd = 0.0846 / pvar)
parm[, "Pcoeff1"] <- qnorm(parm_quant[, "Pcoeff1"], mean = -0.07, sd = 0.07 / pvar)
parm[, "Pcoeff2"] <- qnorm(parm_quant[, "Pcoeff2"], mean = -0.0043, sd = 0.0043 / pvar)
parm[, "intercep"] <- qunif(parm_quant[, "intercep"], min = 0.28 - 0.28 * pvar, max = 0.28 + 0.28 * pvar)
...
The maximum yield is most sensitive to intercept, while the mean yield and min yield are most sensitive to precipitation.
...
result <- map(parm, cor.test, y = yieldsd$maxyield) # cor.test = correlation coefficient
result$Tmincoeff1
result$Tmincoeff2
result$Pcoeff10
result$Pcoeff2
