...
ggplot(msage, 
       aes(as.Date(date),
                  msage[,best_par])) +
  geom_line(col="red") +
  geom_line(data=msage, aes(as.Date(date), obs),
            col="black",
            linetype=2) +
  labs(y="Streamflow",
       x="Date")
...
```{r}
# # streamflow in august (peak summer)
# aug_runoff <- function(m, o, month) {
#   # august <- filter(month == 8)
#   msage$o <- na.omit(msage$o)
#   mean_obs <- mean(o)
#   
#   return(mean_obs)
# }
...
# res <- msage |> 
#   select(-date, -day, -year, -month, -wy, -obs ) |>  
#   map_dbl(aug_runoff, o=msage$mean_obs)
