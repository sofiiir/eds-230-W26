...
```{r}
# now run the model again
site2 <- solarpv(area = 0.1, eff = 0.6,
          solar = sierraczosolar, clr = "hotpink", eunit = "kW", g = TRUE)
```
...
The relative difference is a reduction of photovoltaic energy generation of 25 percent. This is relative to the change in the parameter but this is only true for simple models.
...
site2 <- eff  |>  purrr::map(~ solarpv(area = 0.1, 
  solar = sierraczosolar, clr = "green", 
  eunit = "kWhr", g = FALSE, etype = "direct", eff = .x)) # .x tells the function to get the values from the piped in values
...
# the use of the bracket is referencing the whole list
