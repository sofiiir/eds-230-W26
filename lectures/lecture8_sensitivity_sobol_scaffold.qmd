...
## Sensitivity Analysis Goal {.scrollable}
...
For the $k_d$ and $k_0$ parameters you can assume that they are normally distributed with standard deviation of 10% of their default values
...
v2 <- rnorm(mean = 2.5, sd = .3, n = np)
height2 <- runif(min = 9.5, max = 10.5, n = np) 
...
The order effect is from the `original` column. If the confidence intervals includes zero it isn't having an effect on the output.
...
```{r, alternative}
# scheme = B to get the second order effect
