...
tmp <- w8_wy %>% pivot_longer(cols=c("model","obs"), 
                             names_to="source",
                             values_to="flow")

a <- ggplot(tmp,
            aes(wy, flow, fill=source)) +
...
p1 +
  geom_line(data=subset(msage, wy == 1970),
...
ggplot(subset(msagel, wy == 1970 & month == 6),
       aes(as.Date(date),
           str,
           col=run)) +
  geom_line() +
